...
const CONCEPTS: Record<Locale, { terms: string[]; to: 'family' | 'luxury' | 'glacier' | 'beginner' | 'freestyle' | 'apres' | 'spa' | 'powder' }[]> = {
  ...
  ja: [
    { terms: ['家族向け'], to: 'family' },
    { terms: ['高級', 'ラグジュアリー'], to: 'luxury' },
    { terms: ['氷河'], to: 'glacier' },
    { terms: ['初心者'], to: 'beginner' },
    { terms: ['スノーパーク'], to: 'freestyle' },
    { terms: ['アフタースキー'], to: 'apres' },
    { terms: ['スパ'], to: 'spa' },
    { terms: ['パウダー'], to: 'powder' },
  ],
  'zh-hk': [
    { terms: ['家庭', '親子'], to: 'family' },
    { terms: ['豪華'], to: 'luxury' },
    { terms: ['冰川'], to: 'glacier' },
    { terms: ['初學者'], to: 'beginner' },
    { terms: ['滑雪公園'], to: 'freestyle' },
    { terms: ['après-ski'], to: 'apres' },
    { terms: ['水療'], to: 'spa' },
    { terms: ['粉雪'], to: 'powder' },
  ],
}
...
export function linkifyProse(text: string, d: Destination, l: Locale, max = 6): ReactNode[] {
  ...
  const sioPhrase: Record<Locale, string[]> = {
    ...
    ja: ['ski-in/ski-out', 'スキーインスキーアウト', 'ゲレンデ直結'],
    'zh-hk': ['ski-in/ski-out', '滑雪直達'],
  }
