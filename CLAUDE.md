...
- **6 languages**: en / fr / es / pt / it / nl (nl = Dutch, added 2026-08; pt = European Portuguese post-Acordo)
...
- Available locales: `en`, `fr`, `es`, `pt`, `it`, `nl` (see `app/[locale]/dictionaries.ts`). The locale list is duplicated in `proxy.ts` and `lib/site.ts` (LOCALES) and `components/layout/Header.tsx` (allLocales) - keep them in sync. Adding a locale: run `npx tsc --noEmit`, every `Record<Locale>` missing the key errors and becomes your checklist.
...
2. Provide `intro` (+ `longDescription`, `skiInSkiOutNote`) in all 6 languages
3. Rebuild, and pages, sitemap and OG images regenerate automatically

## SEO
- Every page has `generateMetadata` with hreflang to all 6 locales
...
- Do not break the 6-locale parity (every page must exist in en/fr/es/pt/it/nl or fall back gracefully)
