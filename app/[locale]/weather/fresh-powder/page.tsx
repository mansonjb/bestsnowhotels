...
export async function generateMetadata({
  ...
  return {
    title: `${dict.weather.freshPowderTitle} | BestSnowHotels`,
    description: dict.weather.freshPowderSubtitle,
    alternates: {
      canonical: `${SITE_URL}/${l}/weather/fresh-powder`,
      languages: Object.fromEntries([
        ...locales.map((x) => [x, `${SITE_URL}/${x}/weather/fresh-powder`]),
        ['x-default', `${SITE_URL}/en/weather/fresh-powder`],
      ]),
    },
  }
}

export default async function FreshPowderPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  ...
  const jsonLd = [
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: t.freshPowderTitle, item: `${SITE_URL}/${l}/weather/fresh-powder` },
      ],
    },
    {
      '@context': 'https://schema.org',
      '@type': 'ItemList',
      name: t.freshPowderTitle,
      description: t.freshPowderSubtitle,
    ...
    <>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-6">
        <nav className="text-sm text-ice-700 mb-4">
          ...
          <span className="text-slate-deep">{t.freshPowderTitle}</span>
        </nav>
        ...
        <p className="mt-4 text-lg text-ice-800/80 max-w-3xl">{t.freshPowderSubtitle}</p>
      </section>
