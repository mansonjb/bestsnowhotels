...
const REASON: Record<'champion' | 'gentle' | 'beginner' | 'carfree' | 'sio' | 'tagged', Record<Locale, string>> = {
  champion: { en: 'Renowned for families', fr: 'Référence en famille', es: 'Referente familiar', pt: 'Referência em família', it: 'Icona per le famiglie', ja: '家族向けの定番', nl: 'Bekend bij gezinnen', 'zh-hk': '馳名家庭之選' },
  gentle: { en: 'Gentle slopes', fr: 'Pentes douces', es: 'Pistas suaves', pt: 'Pistas suaves', it: 'Piste dolci', ja: '緩やかなゲレンデ', nl: 'Zachte hellingen', 'zh-hk': '緩坡雪道' },
  beginner: { en: 'Real beginner runs', fr: 'Vraies pistes débutants', es: 'Pistas para principiantes', pt: 'Pistas para principiantes', it: 'Piste per principianti', ja: '本格的な初心者ゲレンデ', nl: 'Echte beginnerspistes', 'zh-hk': '正宗初學者雪道' },
  carfree: { en: 'Car-free village', fr: 'Village sans voitures', es: 'Pueblo sin coches', pt: 'Aldeia sem carros', it: 'Borgo senza auto', ja: '車不要の村', nl: 'Autovrij dorp', 'zh-hk': '無車村莊' },
  sio: { en: 'Ski-in/ski-out', fr: 'Ski au pied', es: 'A pie de pista', pt: 'À beira das pistas', it: 'Sci ai piedi', ja: 'Ski-in/ski-out', nl: 'Ski-in/ski-out', 'zh-hk': 'Ski-in/ski-out' },
  tagged: { en: 'Family favourite', fr: 'Valeur sûre en famille', es: 'Ideal en familia', pt: 'Ideal em família', it: 'Perfetta per famiglie', ja: '家族に人気', nl: 'Favoriet bij gezinnen', 'zh-hk': '家庭首選' },
}
...
const T = {
  h1: (c: string, l: Locale) => ({
    ...
    ja: `${c}の家族向けベストスキー場`,
    nl: `Beste skigebieden voor gezinnen ${inCountry(c, 'nl')}`,
    'zh-hk': `${c}最佳家庭滑雪場`,
  }[l]),
  metaDesc: (c: string, top: string, l: Locale) => ({
    ...
    ja: `${c}の子供・家族向けベストスキー場を、緩やかな地形、初心者向けゲレンデ、そして安全でski-in/ski-outまたは車不要の村を基準にランキング。1位は${top}。`,
    nl: `De beste skigebieden voor kinderen en gezinnen ${inCountry(c, 'nl')}, gerangschikt op zachte hellingen, beginnerspistes en veilige, ski-in/ski-out of autovrije dorpen. ${top} voert onze lijst aan.`,
    'zh-hk': `${c}最佳的兒童及家庭滑雪場，根據緩坡地形、初學者雪道，以及安全、屬於ski-in/ski-out或無車村莊等條件排名，首位為${top}。`,
  }[l]),
  intro: (c: string, n: number, l: Locale) => ({
    ...
    ja: `優れた家族向けスキー場はキッズクラブだけでは終わらない。緩やかで幅の広いゲレンデ、本格的な初心者エリア、そして安心して歩ける村、理想を言えば車不要かski-in/ski-outであること。${c}にあるこれら${n}件のスキー場はまさにその条件で高評価を得ており、いずれもゲレンデ直結のホテルをワンクリックで比較できる。`,
    nl: `Een goed gezinsskigebied is meer dan een kinderclub: het wil brede, zachte pistes, echt beginnersterrein en een dorp dat veilig is om rond te lopen, het liefst autovrij of ski-in/ski-out. Deze ${n} skigebieden ${inCountry(c, 'nl')} scoren daar het best op, elk met hotels aan de piste die je in een klik kunt vergelijken.`,
    'zh-hk': `優秀的家庭滑雪場不僅僅是兒童俱樂部：還需要寬闊平緩的雪道、正宗的初學者區域，以及安全、最好是無車或ski-in/ski-out的村莊。位於${c}的這${n}個滑雪場正正在這些條件上表現最佳，並且每一個都設有可一鍵比較的雪道旁酒店。`,
  }[l]),
  quick: (top: string, region: string, l: Locale) => ({
    ...
    ja: `このリストで家族に一番おすすめなのは${region}の${top}。緩やかな地形、リフトでアクセスできる初心者エリア、そして小さなスキーヤーも宿から近い距離で楽しめるベースが魅力。`,
    nl: `De beste gezinskeuze op deze lijst is ${top}, in ${region}: zacht terrein, makkelijke beginnerszones met liften en een basis die kleine skiërs dicht bij de deur houdt.`,
    'zh-hk': `這份名單中最推薦家庭的是${region}的${top}：地勢平緩，設有纜車直達的初學者區域，而且基地與住宿距離相近，方便小小滑雪愛好者。`,
  }[l]),
  rankTitle: { en: 'The resorts, ranked for families', fr: 'Les stations, classées pour les familles', es: 'Las estaciones, clasificadas para familias', pt: 'As estâncias, classificadas para famílias', it: 'Le località, in classifica per le famiglie', ja: '家族向けスキー場ランキング', nl: 'De skigebieden, gerangschikt voor gezinnen', 'zh-hk': '家庭滑雪場排行榜' } as Record<Locale, string>,
  rankNote: { en: 'Resorts the family-ski press consistently recommends lead the list (marked ★), then our own terrain score for gentle, kid-friendly skiing.', fr: 'Les stations que la presse spécialisée recommande le plus arrivent en tête (repère ★), puis notre score de terrain pour un ski doux, adapté aux enfants.', es: 'Encabezan las estaciones que la prensa de esquí en familia más recomienda (marca ★), luego nuestro score de terreno para un esquí suave y apto para niños.', pt: 'Lideram as estâncias que a imprensa de esqui em família mais recomenda (marca ★), depois o nosso score de terreno para um esqui suave e adequado a crianças.', it: 'Guidano le località che la stampa dello sci in famiglia consiglia di più (segno ★), poi il nostro punteggio di terreno per uno sci dolce e adatto ai bambini.', ja: '家族向けスキー専門メディアが一貫して推薦するスキー場を上位に配置し（★印）、続いて緩やかで子供にやさしいゲレンデかどうかの独自スコアで並べている。', nl: 'De skigebieden die de gezinsski-pers het vaakst aanbeveelt, staan bovenaan (met ★), gevolgd door onze eigen terreinscore voor zacht, kindvriendelijk skien.', 'zh-hk': '家庭滑雪媒體一致推薦的滑雪場排在前列（標示★），其後按我們獨立評分的地形平緩度及是否適合兒童排列。' } as Record<Locale, string>,
  easyPistes: { en: 'easy pistes', fr: 'de pistes faciles', es: 'de pistas fáciles', pt: 'de pistas fáceis', it: 'di piste facili', ja: '初心者向けゲレンデ', nl: 'makkelijke pistes', 'zh-hk': '初級雪道' } as Record<Locale, string>,
  guide: { en: 'Resort guide', fr: 'Guide de la station', es: 'Guía de la estación', pt: 'Guia da estância', it: 'Guida della località', ja: 'スキー場ガイド', nl: 'Gids van het skigebied', 'zh-hk': '滑雪場指南' } as Record<Locale, string>,
  book: { en: 'Find a family hotel', fr: 'Trouver un hôtel familial', es: 'Buscar un hotel familiar', pt: 'Encontrar um hotel familiar', it: 'Trova un hotel per famiglie', ja: '家族向けホテルを探す', nl: 'Zoek een gezinshotel', 'zh-hk': '尋找家庭酒店' } as Record<Locale, string>,
  mapTitle: { en: 'Family hotels around', fr: 'Hôtels familiaux autour de', es: 'Hoteles familiares en torno a', pt: 'Hotéis familiares em redor de', it: 'Hotel per famiglie intorno a', ja: '周辺の家族向けホテル：', nl: 'Gezinshotels rond', 'zh-hk': '周邊的家庭酒店：' } as Record<Locale, string>,
  more: { en: 'Keep planning', fr: 'Continuer à préparer', es: 'Sigue planificando', pt: 'Continue a planear', it: 'Continua a pianificare', ja: '計画を続ける', nl: 'Verder plannen', 'zh-hk': '繼續規劃' } as Record<Locale, string>,
  allCountries: { en: 'Family skiing by country', fr: 'Ski en famille par pays', es: 'Esquí en familia por país', pt: 'Esqui em família por país', it: 'Sci in famiglia per paese', ja: '家族向けスキー（国別）', nl: 'Skien met het gezin per land', 'zh-hk': '家庭滑雪（按國家）' } as Record<Locale, string>,
  europeList: { en: 'Best family resorts in Europe', fr: "Meilleures stations familiales d'Europe", es: 'Mejores estaciones familiares de Europa', pt: 'Melhores estâncias familiares da Europa', it: "Migliori località per famiglie d'Europa", ja: 'ヨーロッパの家族向けベストスキー場', nl: 'Beste skigebieden voor gezinnen in Europa', 'zh-hk': '歐洲最佳家庭滑雪場' } as Record<Locale, string>,
  faqTitle: { en: 'Family skiing questions', fr: 'Questions ski en famille', es: 'Preguntas sobre esquí en familia', pt: 'Perguntas sobre esqui em família', it: 'Domande sullo sci in famiglia', ja: '家族向けスキーのよくある質問', nl: 'Vragen over skien met het gezin', 'zh-hk': '家庭滑雪常見問題' } as Record<Locale, string>,
}
