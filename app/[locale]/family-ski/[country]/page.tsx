...
const REASON: Record<'champion' | 'gentle' | 'beginner' | 'carfree' | 'sio' | 'tagged', Record<Locale, string>> = {
  champion: { en: 'Renowned for families', fr: 'Référence en famille', es: 'Referente familiar', pt: 'Referência em família', it: 'Icona per le famiglie', ja: '家族向けの定番', nl: 'Bekend bij gezinnen' },
  gentle: { en: 'Gentle slopes', fr: 'Pentes douces', es: 'Pistas suaves', pt: 'Pistas suaves', it: 'Piste dolci', ja: '緩やかなゲレンデ', nl: 'Zachte hellingen' },
  beginner: { en: 'Real beginner runs', fr: 'Vraies pistes débutants', es: 'Pistas para principiantes', pt: 'Pistas para principiantes', it: 'Piste per principianti', ja: '本格的な初心者ゲレンデ', nl: 'Echte beginnerspistes' },
  carfree: { en: 'Car-free village', fr: 'Village sans voitures', es: 'Pueblo sin coches', pt: 'Aldeia sem carros', it: 'Borgo senza auto', ja: '車不要の村', nl: 'Autovrij dorp' },
  sio: { en: 'Ski-in/ski-out', fr: 'Ski au pied', es: 'A pie de pista', pt: 'À beira das pistas', it: 'Sci ai piedi', ja: 'Ski-in/ski-out', nl: 'Ski-in/ski-out' },
  tagged: { en: 'Family favourite', fr: 'Valeur sûre en famille', es: 'Ideal en familia', pt: 'Ideal em família', it: 'Perfetta per famiglie', ja: '家族に人気', nl: 'Favoriet bij gezinnen' },
}
...
const T = {
  ...
  rankTitle: { en: 'The resorts, ranked for families', fr: 'Les stations, classées pour les familles', es: 'Las estaciones, clasificadas para familias', pt: 'As estâncias, classificadas para famílias', it: 'Le località, in classifica per le famiglie', ja: '家族向けスキー場ランキング', nl: 'De skigebieden, gerangschikt voor gezinnen' } as Record<Locale, string>,
  rankNote: { en: 'Resorts the family-ski press consistently recommends lead the list (marked ★), then our own terrain score for gentle, kid-friendly skiing.', fr: 'Les stations que la presse spécialisée recommande le plus arrivent en tête (repère ★), puis notre score de terrain pour un ski doux, adapté aux enfants.', es: 'Encabezan las estaciones que la prensa de esquí en familia más recomienda (marca ★), luego nuestro score de terreno para un esquí suave y apto para niños.', pt: 'Lideram as estâncias que a imprensa de esqui em família mais recomenda (marca ★), depois o nosso score de terreno para um esqui suave e adequado a crianças.', it: 'Guidano le località che la stampa dello sci in famiglia consiglia di più (segno ★), poi il nostro punteggio di terreno per uno sci dolce e adatto ai bambini.', ja: '家族向けスキー専門メディアが一貫して推薦するスキー場を上位に配置し（★印）、続いて緩やかで子供にやさしいゲレンデかどうかの独自スコアで並べている。', nl: 'De skigebieden die de gezinsski-pers het vaakst aanbeveelt, staan bovenaan (met ★), gevolgd door onze eigen terreinscore voor zacht, kindvriendelijk skien.' } as Record<Locale, string>,
  easyPistes: { en: 'easy pistes', fr: 'de pistes faciles', es: 'de pistas fáciles', pt: 'de pistas fáceis', it: 'di piste facili', ja: '初心者向けゲレンデ', nl: 'makkelijke pistes' } as Record<Locale, string>,
  guide: { en: 'Resort guide', fr: 'Guide de la station', es: 'Guía de la estación', pt: 'Guia da estância', it: 'Guida della località', ja: 'スキー場ガイド', nl: 'Gids van het skigebied' } as Record<Locale, string>,
  book: { en: 'Find a family hotel', fr: 'Trouver un hôtel familial', es: 'Buscar un hotel familiar', pt: 'Encontrar um hotel familiar', it: 'Trova un hotel per famiglie', ja: '家族向けホテルを探す', nl: 'Zoek een gezinshotel' } as Record<Locale, string>,
  mapTitle: { en: 'Family hotels around', fr: 'Hôtels familiaux autour de', es: 'Hoteles familiares en torno a', pt: 'Hotéis familiares em redor de', it: 'Hotel per famiglie intorno a', ja: '周辺の家族向けホテル：', nl: 'Gezinshotels rond' } as Record<Locale, string>,
  more: { en: 'Keep planning', fr: 'Continuer à préparer', es: 'Sigue planificando', pt: 'Continue a planear', it: 'Continua a pianificare', ja: '計画を続ける', nl: 'Verder plannen' } as Record<Locale, string>,
  allCountries: { en: 'Family skiing by country', fr: 'Ski en famille par pays', es: 'Esquí en familia por país', pt: 'Esqui em família por país', it: 'Sci in famiglia per paese', ja: '家族向けスキー（国別）', nl: 'Skien met het gezin per land' } as Record<Locale, string>,
  europeList: { en: 'Best family resorts in Europe', fr: "Meilleures stations familiales d'Europe", es: 'Mejores estaciones familiares de Europa', pt: 'Melhores estâncias familiares da Europa', it: "Migliori località per famiglie d'Europa", ja: 'ヨーロッパの家族向けベストスキー場', nl: 'Beste skigebieden voor gezinnen in Europa' } as Record<Locale, string>,
  faqTitle: { en: 'Family skiing questions', fr: 'Questions ski en famille', es: 'Preguntas sobre esquí en familia', pt: 'Perguntas sobre esqui em família', it: 'Domande sullo sci in famiglia', ja: '家族向けスキーのよくある質問', nl: 'Vragen over skien met het gezin' } as Record<Locale, string>,
}
